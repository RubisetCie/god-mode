    ...
    for (i = 1; i < argc; i++)
    {
        ...
        if ((argv[i][0] == L'/' || argv[i][0] == L'-') && argv[i][1] != L'\0' && argumentStart == 0)
        {
            ...
            switch (option[1])
            {
                ...
                /* Execute as a command (by creating a console environment) */
                case 'm':
                case 'M':
                    console = TRUE;
                    break;
